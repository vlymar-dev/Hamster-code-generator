...
from bot.handlers.games_keys import get_games_handler
...
async def test_info_handler_success(
    telegram_user,
    telegram_chat,
    ...
    callback_query = telegram_callback_query(chat=telegram_chat, user=telegram_user(123), data='get_games')

    mock_image_manager.get_random_image = MagicMock(return_value=return_value)

    await get_games_handler(callback_query=callback_query, image_manager=mock_image_manager)
    ...
    assert callback_query.data == 'get_games'
