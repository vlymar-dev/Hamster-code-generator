...
async def test_info_handler_success(
    ...
    callback_query = telegram_callback_query(chat=telegram_chat, user=telegram_user(123), data='user_info')

    mock_image_manager.get_random_image = MagicMock(return_value=return_value)

    await user_info_handler(callback_query=callback_query, image_manager=mock_image_manager)
    ...
    assert callback_query.data == 'user_info'
