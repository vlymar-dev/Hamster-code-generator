...
async def referral_links_handler(callback_query: CallbackQuery, image_manager: ImageManager) -> None:
    ...
    try:
        ...
        response_text = _(
            '💎 <b>Join now and unlock exclusive bonuses!</b> '
        ...
        if image:
            await callback_query.message.answer_photo(
                photo=image, caption=response_text, reply_markup=referral_links_kb()
            )
        else:
            logger.warning(f'No images available in referral links for user {user_id}')
            await callback_query.message.answer(text=response_text, reply_markup=referral_links_kb())
